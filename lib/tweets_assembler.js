...
var Renderer = {
  ...
  openImageSource: function(entity) {
    return function() {
      if(entity.video_info) {
        ...
        openTab(entity.media_url_https);
      }
    }
  },
  ...
  renderTweet: function (tweet, useColors, nameAttribute, topLevel) {
    ...
    if(tweet.quoted_status) {
      ...
      var linkDst = TwitterLib.URLS.BASE + tweet.quoted_status.user.id + '/status/' + tweet.quoted_status_id_str;
      quoting = document.createElement("a");
      quoting.setAttribute("href", "#");
      quoting.appendChild(Renderer.makeText(chrome.i18n.getMessage("quoting") + ' ' + tweet.quoted_status.user.screen_name));
      var replyAction;
      if(this.isNotification()) {
        replyAction = function() {
          openTab(linkDst);
      ...
      Renderer.expandQuote(tweet, tweetSpace, true, topLevel && OptionsBackend.get('auto_expand_quotes'));
    }
  ...
  expandQuote: function(tweet, target, showIfVisible, autoExpand) {
    if(showIfVisible && !tweet.quoteVisible && !autoExpand) {
      return;
    }

    tweet.quoteVisible = true;
    var renderedTweet = Renderer.renderTweet(tweet.quoted_status, false, OptionsBackend.get('name_attribute'));
    target.appendChild(renderedTweet);
    var separator = Renderer.makeDiv({'class': 'reply_separator'}, "\u2193");
    renderedTweet.insertBefore(separator, renderedTweet.childNodes[0]);
    separator.onclick = function() {
