var Composer = {
  ...
  reply: function (node) {
    ...
    if((replies.indexOf('@' + ownName) >= 0) && replies.length > 1) {
      replies.splice(replies.indexOf('@' + ownName), 1);
    }
  ...
  textareaChanged: function (e) {
    ...
    var availableChars =
      MAX_TWEET_SIZE - twttr.txt.getTweetLength(el.val(), configurationData.val());
    var charsLeftEl = $("#compose_tweet_area .chars_left");
    charsLeftEl.text(availableChars);
    var reason = twttr.txt.isInvalidTweet(el.val(), configurationData.val());
    if(reason) {
      if(availableChars < 0) {
    ...
    switch(reason) {
      ...
      case "invalid_characters":
        title = chrome.i18n.getMessage("ir_" + reason);
        break;
    }
    if(title) {
      $("#compose_tweet_area input[type='button']").attr("title", title);
    } else {
      $("#compose_tweet_area input[type='button']").removeAttr("title");
    }
  },
